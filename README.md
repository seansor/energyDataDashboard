...
This section gives a breakdown of the TFC in terms of sector and fuel type. The user is first presented with the most recent
full datasets available which are for 2017. This data is illustrated for the user through the use of pie-charts which are 
suitable for displaying data for a single year. The user can filter the data by clicking on the different data types directly
in the charts or by clicking on the legend items. The user has the ability to select different timescales which change the type
...
over time as well as a breakdown by sector and fuel type. For these chart types the user can filter the data by using the
...
not to make the dashboard too "text heavy", much of the text is hidden from view by default. Users can opt to view it or
...
* As a user, I want to be able to see a population graph, so that I can view changes in the population over time.
...
* As a user, I want to be able to be able to interact with the data points, so that I can view the values of each data point.
...
* As a user, I want to view the greenhouse gas emissions in terms of CO2 equivalent for each distinct sector.
...
### Existing Features
The Population and GNI graphs allow users to see the discrete values for each data point by hovering on the data points.

The key statistics are linked to the datasets and are rendered via graph.js using dc.js "number display" functionality.
(Exception: Greenhouse gas emissions key stats which are not related explicitly to the data and are just informative and are
therefore hard coded into the html).

The 'Select Years' feature in the 'Energy' section allows users to view different date ranges (2017, 10yr, max) by 
selecting the relevant date range from the select options. This feature displays a different graph type for each option.
The select menu also hides or displays additional sector/fuel select menus depending on the option selected. This functionality
...
hovering over the data of interest in the graphs. The Energy values are displayed in "kilo-tonne of oil equivalent" (ktoe).

The select menu in the greenhouse gas section allows the users to change date range by selecting the relevant option. As per
...
The user can see the discrete value for the greenhouse gas emissions associated with any of the sectors by hovering over the data
of interest in the graphs. The emissions values are displayed in "kilo-tonne of CO2" equivalent (ktCO2).

Throughout the dashboard the user can see more or less text (graph information) by clicking on the "more info/less info" text provided.

### Future additional Features
In the future a data on Primary Energy may be incorporated into the dashboard.

A more detailed breakown of CO2 emissions including a more granular breakdown of emissions by sector
...
(see below) support which allows highly efficient exploration on large multi-dimensional datasets. It leverages
...
d3.js libraries for interaction with the data in the browser. Due to its design it is extremely fast and is particularly
...
* Additional select menus for sector and fuel type hidden when graphs are pie charts (i.e. 2017 selected).
...
necessary to add a listener and call the function each time a change in screen size is detected as the charts need to be
...
as "master branch" >> clicked on the link that appears that says "Your site is published at
...
## Credits
### Content
Parts of the text describing the population and GNI statistics were copied from the Central Statistics Office (CSO) Website:
